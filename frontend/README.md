...
- **제출본 3면은 `일시 | 행위 주체 | 요약 · 금액` 세 열입니다** (`PdfBuilder.addPage3`와 같은 구성).
  ...
  주체·금액을 넣지 않습니다
- **소명서 생성이 실패하면 그 화면으로 넘기지 않습니다.** 넘기면 문장 0개인 화면이 성공한
  것처럼 보이고, 그대로 내보내면 **1·2면이 "확인된 사실관계가 없습니다"로 찍힌 서류가 은행에
  갑니다**(백엔드가 세션에 문장이 없을 때 쓰는 정직한 문구라, 서류만 보면 원인을 알 수 없습니다).
  `draftShown`을 안 올려 `[초안 만들기]`가 남고 다시 시도할 수 있게 둡니다. 문구도 따로 씁니다 —
  `runLive(call, failMessage)`의 두 번째 인자입니다. 기본 문구는 오류 **코드** 단위라 여러 호출이
  나눠 쓰는데, 같은 `TIMEOUT`이어도 자료 판독 실패("읽은 것만 먼저 보여드릴게요")와 소명서 생성
  실패는 사용자가 할 일이 다릅니다. **세션 만료만 예외** — 화면이 처음으로 돌아가므로 그 사실을
  말해주는 쪽이 맞습니다
- **버튼 색이 역할을 나눕니다.** `bg-brand`(파랑)는 **하단 CTA 전용**이라 "다음 단계로"만
  뜻합니다 — 단계마다 뜻이 흔들리면 그 버튼을 못 믿게 되므로, 화면의 행동을 여기에 얹지
  않습니다. `bg-ink`(검정)가 **그 화면에서 하는 일**이고(증거정리 `자료 조립하기`, 소명서
  `제출 패키지 내보내기`), 나머지는 테두리만 씁니다. 소명서 화면에서 내보내기가 한동안
  테두리 버튼이라, 화면에서 가장 강한 버튼이 "접수 안내 보기"였습니다 — **결과물보다 다음
  단계가 무거워 보이면 파일을 못 받고 나가는 길이 생깁니다**
- **5단계는 파일을 안 받은 사람을 잡습니다** (`RoutesStage`). `packageConfirmedAt`이 `null`이면
  맨 위에 "아직 제출 파일을 안 받으셨어요 · [지금 받기]" 한 줄이 뜨고, 거기서 바로
  내보내기를 엽니다(4단계로 돌려보내지 않습니다). **막지 않습니다** — 확인 대화상자로 세우면
  안내를 먼저 읽으려던 사람까지 멈추고, 되돌릴 수 있는 행동에 확인을 붙이면 "확인창은 진짜
  위험할 때만 뜬다"는 신뢰가 옅어집니다. 도착하기 전이 아니라 **도착한 자리**에 두는 이유는,
  사람이 "다 했다"고 느끼는 지점이 거기이고 단계 인디케이터로 건너뛴 사람도 같이 걸리기 때문입니다
