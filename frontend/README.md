...
- **고친 문장을 지우지 않습니다.** 근거와 매칭되지 않아도 경고만 띄우고 문장은 살립니다.
  ...
  아닙니다 (FR-045 ③ 개정)
- **반복 거래는 `recurrence`로 카드 한 장이 됩니다.** AI-server가 12개월 자동이체를 한 장으로
  묶고, **`amount`는 1회분 · `occurred_at`은 첫 회차**입니다. 이 사실을 화면에 안 적으면
  12개월치가 6만 원짜리 1회 거래로 보이고 사용자가 그대로 확인하고 넘어갑니다. 세 자리에 씁니다 —
  확인 카드("매월 · 12회" 배지 / "금액 (1회분)" / 기간 한 줄), **제출본 3면**(요약 뒤 `(매월 12회)`,
  금액 뒤 `(1회분)`), **4면**(카드가 **하나뿐일 때만** 전체 기간 + `(매월 12회, 1회분 …)`).
  3·4면 문구는 **서버 PDF와 같아야 해서** `lib/api/adapt.ts`의 `recurrenceCountLabel`·
  `formatAmountText`를 미리보기와 공유합니다 — 여기가 갈리면 미리보기와 내려받은 파일이 달라집니다
- **제출본 3면은 `일시 | 행위 주체 | 요약 · 금액` 세 열입니다** (`PdfBuilder.addPage3`와 같은 구성).
  한동안 미리보기가 일시·요약 둘만 그려서 금액이 아예 안 보였습니다 — 같은 면인데 미리보기만
  열이 적으면 "PDF가 단일 출처"가 깨집니다. 화면 타임라인(`toTimelineEvents`)은 별개라
  주체·금액을 넣지 않습니다
