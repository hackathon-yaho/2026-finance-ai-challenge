...
**아직 목인 것 중 눈에 띄는 하나** — 카드·문장의 `[원본 보기]`는 `ViewerSheet`가 그리는
**재현 화면**이지 사용자가 올린 이미지가 아닙니다. F7-05가 정한 실제 동작은
"`imageIndex`로 자기 메모리의 blob 배열에서 해당 이미지를 찾아 표시"이고, 지금 목 카드의
`source_image_index`는 올린 장수와 무관하게 0~4로 고정이라 연결하면 **없는 출처를 주장하게
...
- **제출본에는 확인된 카드만 싣습니다.** 화면 타임라인(`flow.timeline`)과 제출본 3면
  ...
  다 실으면 같은 묶음 안에서 어긋납니다. **3면 규칙은 아직 명세에 없어** 백엔드에 확인을
  요청해 두었습니다 ([page3-and-biz-notice.md](../docs/request/backend/page3-and-biz-notice.md))
