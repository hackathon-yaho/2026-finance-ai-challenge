  ...
  return (
    ...
    >
      ...
      {/* 막힌 이유를 구분해 적는다. 연도 없는 캡처(`occurred_at == null`)는 판독이 틀린 게
          ...
          **날짜가 값을 가진 채 막히면 그 값을 그대로 묻는다** (2026-08-28, AI 연도 추론
          B안). 확인할 값이 화면에 있는데 "신뢰도가 낮아요"라고만 하면 사용자는 무엇을
          봐야 하는지 모른다. 바로 위 날짜 칸과 같은 값이지만, 물음이 붙어야 눈이 간다. */}
      {blocking && (
        <p className="mt-2.5 text-xs leading-normal text-warning">
          {blockingReason(card) === "date_missing"
            ...
            : blockingReason(card) === "date_low"
              ? `${formatWhen(card.occurred_at)} 맞나요? 화면에서 날짜를 다 읽지 못해 추정한 값이에요.`
              : "금액 판독 신뢰도가 낮아요. 이 카드를 확인해야 다음 단계로 갈 수 있어요."}
        </p>
      )}
