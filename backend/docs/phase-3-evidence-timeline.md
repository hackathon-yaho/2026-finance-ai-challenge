...
| 상태 | 처리 |
| --- | --- |
| **날짜 또는 금액이 `low` 신뢰도인 카드가 미확인** (값이 `null`이 아닌 경우에 한함) | 프론트 차단 + **백엔드도 `/api/readiness`를 `409` + `UNCONFIRMED_FIELDS`로 거부** |
...
- [ ] **`amount`/`occurred_at`이 `null`인 카드의 `field_confidence`는 게이팅 판단에서 읽지 않는다** (2026-08-25 ② 확정, AI 회신). `amount == null`(금액 못 읽음)은 이미 별도 신호이고, 그 카드에 채워지는 `low`는 의미 없는 값이다 — 상세: `../../docs/02-architecture/internal-api-contract.md` "신뢰도의 `null`" 절
