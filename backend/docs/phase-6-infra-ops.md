...
- [ ] `ai-server/demo/*.json` → `src/main/resources/demo/`로 복사. **⚠️ v1.1을 받는다** — AI가 `field_confidence` 버그를 픽스처 18곳에서 고쳤고 `박OO`(부분 마스킹 형태)을 `박서준`으로 바꿨다. v1을 이미 복사했다면 다시 받는다
- [ ] **Phase 3 `AiClient` 단위 테스트 픽스처로도 쓴다.** 이게 앞당겨 납품된 이유이므로 마지막에 몰아서 쓰지 않는다
- [ ] TC 매핑 방식은 백엔드 재량이다 — 파일명 그대로 쓰거나, 문진과 무관하게 고정 세트를 돌려도 된다. **F11-03의 "혼합 금지 · 배지 표시"만 지키면 된다**
- [ ] 파일에는 이번 회신의 스키마 확장(`source_type`·`counterparty_name`·`payer_name`, `checklist: []`, `amount_mismatch: false`)이 **선반영**돼 있다. 계약과 어긋나면 **계약 문서가 우선**이고, AI 담당이 파일을 맞춰준다
...
- [ ] **백업 모니터 1개를 추가로 등록**한다(UptimeRobot 등) — cron-job.org는 제3자 무료 서비스라 SLA가 없다. 하나가 지연돼도 다른 쪽이 메운다
- [ ] 두 모니터 모두 **실패 시 이메일 알림**을 켠다
