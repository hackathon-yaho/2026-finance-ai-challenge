...
FORBIDDEN_PHRASES = [
    ...
    # 결과 예측·낙관 일반형. reason-type-rules.md §4와 TC-04가 "가능성이 높습니다"를
    # 명시적으로 금지한다. 특정 어휘(해제/승인)만 막으면 "인용될 가능성이 높습니다"처럼
    # 같은 예측을 다른 단어로 쓴 문장이 그대로 통과한다 — 평가 세트에서 실제로 발견됨.
    "가능성이 높", "가능성이 큽", "가능성이 큰", "확률이 높", "확률이 큽",
    "인용될", "인용됩니다", "인용 가능", "받아들여질", "유리하게 작용", "유리할 것",
    "선처", "간곡", "억울", "호소",
    "배송 완료", "배송이 완료", "배송을 완료", "수령을 확인", "수령하였", "수령했", "수령 완료",
    "편취 의도", "증명합니다", "증명한다", "입증합니다", "입증한다",
    "명백합니다", "명백히", "분명합니다", "확실합니다", "틀림없",
    # 과거 지급정지 이력 — history는 애초에 전달되지 않지만 새어나가면 사용자에게 불리하다 (TC-29)
    "지급정지 이력", "지급정지된 이력", "과거 지급정지", "이전 지급정지", "이전에도 지급정지", "과거에도 지급정지",
    # 금액 평가 — '소액' 기준은 은행 내규로 비공개다. 추정 금지 (PRD §14 OI-01)
    "소액이므로", "소액이어서", "소액에 불과", "소액인 점", "금액이 크지 않", "큰 금액이 아", "적은 금액",
    # 이름 대조 판정 — 대조는 백엔드가 한다. 삼각사기 피해자는 원래 불일치한다 (TC-25)
    "일치하지 않", "불일치", "명의가 다르", "동일인이 아니",
]
