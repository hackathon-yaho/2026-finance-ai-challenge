...
**AI-server는 완전 무상태(stateless)다.** 세션 해시를 받지 않고, 요청 하나를 처리해 응답하면 아무것도 남지 않는다. 상태는 전부 백엔드 세션의 몫이다. 이 성질이 무저장 원칙(`privacy-and-safety.md`)의 AI 쪽 구현이자, 컨테이너가 언제 재기동돼도 안전한 이유다.
...
| 웹 프레임워크 | **FastAPI + uvicorn** | async 동시 처리(LLM 대기 시간 동안 논블로킹), pydantic 기반 스키마 검증(계약=코드), 메모리 풋프린트가 작아 Cloud Run 1GiB에 여유 |
...
| 배포 | **Google Cloud Run** (Docker) | 2026-08-25 변경, 종전 Render Starter. 무료 한도 안에서 $0 — §10 |
...
- **연도 없는 캡처의 연도 추론** (2026-08-27 B안). `reference_date`가 오면 연도를 채우되 **추론값은 신뢰도 `low`로 강제**해 FR-028 게이팅을 유지한다. 기준 시점이 없으면 추론하지 않는다. 미래 날짜는 코드가 −1년(`_apply_year_guard`)
- **반복 카드 중복 접기** (`_dedupe`) — 같은 시리즈를 두 번 낸 카드를 접는다. 반복 카드에만 적용(비반복은 실제로 두 건일 수 있다)
...
시스템 프롬프트는 **고정 문자열**(요청마다 바이트 동일 — OpenAI의 프롬프트 자동 캐싱 전제)로 둔다. PRD §10.1의 7개 규칙에서 출발해 **회신·실측으로 확정된 조항이 계속 더해졌다**(2026-08-27 기준 12개 조항 + 세부 항).

**여기에 프롬프트 전문을 복사해 두지 않는다.** 종전에는 복사해 뒀는데 조항이 늘 때마다 문서만 낡았다 — 실제로 "7개 규칙"이라고 적힌 채 12개가 된 상태로 며칠 방치됐다. 조항 목록이 필요하면 코드를 보는 것이 항상 정확하다.

주요 조항의 **근거**만 여기 남긴다:

| 조항 | 왜 있는가 |
| --- | --- |
| ISO 8601 강제 (7-1) | 실 LLM이 `2026.8.19 오전 10:07`로 내놨다. 계약은 ISO이고, 어긋나면 FactChecker 날짜 대조가 통째로 무력화된다 |
| 연도 추론 (7-2·7-3) | `reference_date`가 있을 때만 추론하고 표기한다. 없으면 `null` — 재료 없이 추측하지 않는다 |
| 이름 필드 경계 (9-1) | 이미지 안 지시문이 `payer_name`에 통째로 섞여 들어온 적이 있다. 이 값은 구매자–송금인 대조에 쓰인다 |
| summary는 요약 (9-2) | 원문 복사라 제3자 이름·인젝션 문구가 그대로 실렸다 |
| 협박 오탐 경계 (10) | "환불 안 해주면 신고할게요"를 threat로 분류한 적이 있다 — 재현율만 재면 안 보이는 실패다 |
| 반복 묶기 (11·11-1·11-2) | 자동이체 12개월이 카드 12장이 되던 문제. 같은 날 반복은 정기 거래가 아니므로 제외 |

**프롬프트만으로 지키지 않는 것들** — 아래는 코드가 결정적으로 보장한다(프롬프트 준수는 확률적이라 불변식이 되지 않는다):
`recurrence.count` 계산 · 같은 날 반복 제외 · 미래 연도 −1년 · 추론 연도의 신뢰도 `low` · 흐린 카드 신뢰도 `low` · 이름 필드 오염 제거 · `amount` 절대값 · 반복 카드 중복 접기

→ **전문은 `app/llm/prompts.py`의 `EXTRACT_SYSTEM` / `DRAFT_SYSTEM`이 단일 출처다.**
...
| `PORT` | `8000` | Cloud Run이 주입(기본 8080). Dockerfile이 `${PORT:-8000}`으로 받는다 |
...
합성 이미지는 스크립트(PIL)로 생성한 가짜 대화·거래내역 화면 + 열화 처리(흐림·잘림)로 만든다. **실제 개인 캡처는 평가 세트에 넣지 않는다.**

**구현 완료 (2026-08-25)** — `evals/` 23건. 상세는 `../evals/README.md`.

- **기대값을 이미지와 같은 명세(`cases.py`)에서 뽑는다.** 따로 관리하면 반드시 어긋나고, **기대값이 틀린 평가 세트는 없느니만 못하다** — 모델이 맞게 읽어도 실패로 찍혀 멀쩡한 프롬프트를 고치게 된다.
- **협박은 재현율뿐 아니라 오탐도 잰다.** 재현율만 보면 "전부 협박"이라 답해도 100%가 나오므로, 일반 독촉·공식 안내 문자를 협박으로 분류하면 실패하는 케이스를 넣었다.
- **안전 지표(`근거 없는 문장 비율`·`근거 연결률`)는 LLM 없이 측정한다.** 이 둘의 책임 주체가 FactChecker(결정적 코드)이므로, 적대적 문장을 주입해 걸러지는지 보면 진짜 측정이 된다. `python -m evals.runner --offline`.
- 추출 지표는 배포·LLM 공급자 확정 후 실측해 `evals/README.md` 이력 표에 남긴다.
