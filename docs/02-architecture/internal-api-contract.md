...
> **수정 기록 (2026-08-27 ②, AI)** — 프론트 요청 `../request/ai/duplicate-cards-and-year-inference.md` 반영
> - **연도 없는 캡처의 연도 추론을 B안으로 전환.** `reference_date`가 주어지면 연도를 채우되, **추론값은 신뢰도 `low`라 FR-028 게이팅이 그대로 걸립니다** — 게이팅은 하나도 풀리지 않습니다. 미래 날짜는 코드가 1년을 뺍니다. 실측으로 날짜 정확도 유지 확인(96.7%)
> - **반복 카드 중복 접기** 신설 — 같은 시리즈를 두 번 낸 카드를 AI-server가 접습니다
> - 소명서 타임아웃: `llm_timeout_draft` 10 → 25초. **백엔드 클라이언트 타임아웃 30초 상향 필요** (`../response/backend/draft-timeout-needs-headroom.md`)
...
#### 연도 없는 캡처의 연도 추론 (2026-08-27 확정 — B안 채택)

국내 은행 앱은 올해 거래를 `08.19`처럼 **월·일만** 찍습니다. 예외가 아니라 **기본 경로**입니다. 종전 A안(추론 금지)에서는 이 경우 `occurred_at`이 전부 `null`이 되고, 그러면 **반복 묶기까지 통째로 꺼져** 카드 수가 늘고 3면 타임라인이 "시각 미상"으로 도배됐습니다(프론트 실측: 8장 전부 `null`·전부 차단·중복 4장).

**`reference_date`가 주어지면 연도를 추론합니다.** 다만 세 가지가 함께 걸려 있어야 성립합니다.

| # | 규칙 | 지켜지는 것 |
| --- | --- | --- |
| ① | **`reference_date`가 없으면 추론하지 않습니다** — `occurred_at`은 `null` | 재료 없이 추측하지 않는다는 원칙 그대로 |
| ② | 추론 결과가 기준 시점보다 **미래면 1년을 뺍니다** | 은행 거래내역은 과거만 찍힙니다. 연말연시 경계(1월에 보는 `12.28` → 작년)가 이 가드로 풀립니다. **확률적 판단이 아니라 코드 가드**입니다 |
| ③ | 추론한 값은 **`field_confidence.occurred_at`이 항상 `low`** | **FR-028 게이팅이 그대로 걸립니다** — 사용자가 반드시 확인합니다. 게이팅은 하나도 풀리지 않습니다 |

**추론하는 것은 연도뿐입니다.** 월·일·시각은 화면에 보이는 값 그대로입니다.

**바뀌는 것은 사용자가 보는 문장뿐입니다** — "언제인지 화면에 안 나와 있어요"(무엇을 할지 모름) → "2026년 8월 19일 맞나요?"(확인하거나 고치면 끝).

> **실측 근거** (`../../ai-server/evals/` `ev-noyear-01`·`ev-noyear-02`, 2회 연속): 연도 추론을 켠 뒤에도 날짜 정확도 **96.7%**, 확인 전 오류 차단 **위반 0건**. 기준 시점을 주지 않은 케이스는 `null`을 유지했습니다.

#### 반복 카드 중복 접기 (2026-08-27)

**같은 반복 시리즈를 두 번 낸 카드는 AI-server가 접습니다** (프론트 신고: 3회 중 1회 재현, 중복 카드가 양쪽 다 `recurrence` 보유). 접는 키는 `(source_type, amount, recurrence.first, recurrence.last)`입니다.

**반복 카드에만 적용합니다.** 반복이 아닌 카드는 같은 시각·같은 금액이 실제로 두 건일 수 있어(같은 분에 찍힌 입금 2건) 접으면 사실이 사라집니다.
...
| `/internal/draft` 타임아웃 | 30초 (2026-08-27 상향 — AI-server 1회 시도 상한 25초 + 여유) |
| 백엔드의 재시도 정책 | 1회, 동일 요청 재전송. 실패 시 오류 응답을 그대로 프론트에 전달. `/internal/draft`는 최악 50초(25+25)까지 걸릴 수 있음을 감수하고 유지(2026-08-27 백엔드 결정) |
