...
>
> **✅ AI 후속 처리 완료 (2026-08-25 ②)** — 회신의 요청 3건 전부 반영했습니다. 백엔드가 추가로 할 일은 없습니다.
> - **회신 §6 (`field_confidence` 기본값)**: 지적이 맞습니다. 제안하신 두 안 중 **`Confidence | None = None`(값이 없으면 신뢰도도 없음)** 으로 갔습니다. 다만 **LLM에게 "값이 없으면 신뢰도를 비우라"고 시키지 않고, 후처리에서 결정적으로 덮어씁니다** — 프롬프트 준수는 확률적이라 불변식이 안 됩니다(이름 마스킹을 기각한 것과 같은 이유). `occurred_at`/`actor`/`amount`는 프론트가 F4-06에서 항상 배지로 렌더하는 값이라 **3값을 유지**했고, 대신 **"값이 `null`인 필드의 신뢰도는 읽지 않는다"** 는 해석 규칙을 계약에 적었습니다 — `amount`가 `null`인 카드의 신뢰도로 FR-028 차단을 판단하지 마세요(금액 미판독의 단일 출처는 `amount == null`입니다). 계약: `../../02-architecture/internal-api-contract.md` "신뢰도의 `null`" 절
> - **회신 §1 (과거 이력 서술 금지 / 금액 평가 금지)**: 프롬프트 조항으로 넣되 **거기서 그치지 않고 결정적 금칙어 검사에도 넣었습니다.** 이 두 문장은 한 번만 새어나가도 사용자가 불리한 문서를 제출하게 되므로 확률적 방어만으로는 부족합니다. 이름 대조 판정 문장(TC-25)도 같은 계열로 함께 차단합니다. 회귀 테스트 포함
> - **데모 세트 v1.1**: 같은 `field_confidence` 버그가 픽스처 18곳에 있어 정정했고, `박OO`(부분 마스킹 형태)을 `박서준`으로 바꿨습니다 — **복사하기 전에 다시 받아 가세요.** 앞으로 계약과 어긋나면 AI 쪽 테스트(`ai-server/tests/test_demo_set.py`)가 깨지도록 해 뒀습니다
