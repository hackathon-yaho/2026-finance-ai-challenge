...
| [client-timeouts-in-contract.md](client-timeouts-in-contract.md) | ⏳ **회신 대기** (2026-08-29) | **공개 계약에 타임아웃 절이 없습니다.** 8/27 `/internal/draft` 15 → 30초 상향을 프론트가 못 따라가 **서버는 성공하는데 화면만 실패하는 구간이 10초** 있었습니다(프론트 값은 35초로 정정 완료, `7ffc095`). `api-contract.md`에 엔드포인트별 서버 상한 ↔ 프론트 클라이언트 타임아웃 표 신설 요청 + 확인 2건(`/api/package/text` 생성 상한, `/api/draft/revise`의 LLM 호출 여부) |
