...
| [cross-image-duplicates-and-extract-anchor.md](cross-image-duplicates-and-extract-anchor.md) | ⏳ 회신 대기 (2026-08-27) | **§1** 같은 정기 거래가 입출금 내역·자동이체 내역에 겹쳐 찍혀 카드가 3장이 되는데 `mergeCandidates: 0` — 규칙 범위 재검토 요청. **§2** `/internal/extract`에 기준 시점(오늘 날짜·`intake.when`) 전달 — AI `duplicate-cards-and-year-inference.md` §2의 전제 |
