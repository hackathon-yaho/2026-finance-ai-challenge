...
> 회신은 `../../response/backend/evidence-timeline-schema-additions.md`에 들어옵니다.
> **막고 있는 작업**: 없음 — 백엔드는 이미 이 값들로 구현·테스트를 마쳤습니다. 다만 로드맵상 지금이 프론트도 F3-01(업로드)·F4-06(카드 확인)·F5-03(공백) 화면을 만드는 기간(8/25~8/28)이라, **다른 가정으로 이미 만들고 계셨다면 지금 맞추는 게 나중에 다시 만드는 것보다 쌉니다.**
