...
> **수정 기록 (2026-08-26, AI)** — 문서 지도에 `../ai-server/evals/` 추가. PRD §1.4 AI 지표의 실측 근거(합성 캡처 26장·기대값·채점 규칙)가 저장소에 있는데 지도에 없어 찾으려면 우연히 발견해야 했습니다. **누구나 열어서 검증할 수 있어야 하는 자료입니다.**

> **수정 기록 (2026-08-25 ②, 백엔드)** — AI 회신 4건 반영
> - 문서 지도에 `../ai-server/docs/*`(AI 실행 계획)과 `../ai-server/demo/*`(데모 응답 세트 v1) 추가
> - 개인정보 원칙에 **예외 하나가 생겼습니다** — 거래 당사자(대화 상대·입금자)의 화면 표시명은 원문 추출합니다. 금감원 표준의 "구매자–송금인 일치 확인" 때문입니다. 경계·근거는 `03-infra-ops/privacy-and-safety.md` "추출 범위 예외" 절이 단일 출처입니다

> **수정 기록 (2026-08-25)**
> - 갱신 규칙에 **파생 문서 동기화** 항목 추가 — 공용 문서를 고치면 각자의 실행 계획 문서(`../backend/docs/` 등)도 같은 작업 안에서 확인합니다. 이 규칙이 없어 백엔드 문서 6개가 하루치만큼 낡았던 일이 있었습니다
> - 문서 지도에 `00-context/법정서식-카카오뱅크배포본-*.pdf` 추가

> **수정 기록 (2026-08-24)**
> - **별지 제4호서식 원본 PDF와 대조 문서 추가** (`00-context/법정서식-*`). 웹 조사로 확인한 사실: ① 은행 자체 템플릿은 없고 **법정 서식을 재배포**하는 구조 ② 서식 현행 버전은 `개정 2016. 7. 26.` ③ 이의제기 기한은 **2개월**이 맞음 ④ **우리 계약 8필드가 서식의 11필드와 맞지 않음**(휴대전화번호·전자우편주소·명의인 누락) ⑤ 서식에 **서명날인란**이 있음
> - 갱신 규칙에 **회신 상태 배너**를 추가 — 요청 문서 맨 위에 ⏳/✅/⚠️로 회신 여부를 표시합니다. 기존 요청 4건에 모두 달았습니다
> - 문서 지도에 `../backend/docs/api-spec.md`(백엔드 API 명세서) 추가 — **프론트가 API를 붙일 때 보는 문서**입니다. 백엔드가 API를 완료·수정할 때마다 갱신하며, 계약(`02-architecture/api-contract.md`)과 다르면 계약이 우선입니다
...
| `00-context/법정서식-별지제4호서식-안내.md` | **이의제기신청서 법정 서식 실물 대조** — 서식 구성, 우리 계약과의 필드 차이, 구현 시 지킬 것 | BE (F8-01 구현), FE (서식 폼) |
| `00-context/법정서식-별지제4호서식-이의제기신청서.pdf` | 국가법령정보센터 원본 서식 (**수정 금지**) | 전원 |
| `00-context/법정서식-카카오뱅크배포본-이의제기신청서.pdf` | 은행 배포본 — 수신처만 다르고 나머지는 원본과 동일 (**수정 금지**) | 전원 |
...
| `../backend/docs/api-spec.md` | **백엔드 API 명세서 (구현 현황·요청/응답 예시)** — 프론트가 붙일 때 보는 문서. 계약과 다르면 `02-architecture/api-contract.md`가 우선 | **FE**, BE |
| `../backend/docs/*` | 백엔드 실행 계획(Phase 1~6) 및 결정 로그 | BE |
| `../ai-server/docs/*` | AI-server 설계·실행 계획 (2026-08-25 신설) | AI |
| `../ai-server/demo/*` | **오프라인 데모 모드용 사전 응답 세트 v1** — 백엔드가 `src/main/resources/demo/`로 복사해 사용. 원본 관리는 AI | BE, AI |
| `../ai-server/evals/` | **AI 품질 평가 세트 (F11-05)** — 합성 캡처 26장 + 기대값(`cases.py`) + 채점 러너(`runner.py`). **PRD §1.4 지표의 실측 근거가 여기 있습니다.** 결과·한계는 [`evals/README.md`](../ai-server/evals/README.md) | 전원 |
...
- **요청 문서 맨 위에 회신 상태 배너를 답니다** (⏳ 회신 대기 / ✅ 회신 완료 / ⚠️ 일부 회신). 요청만 쌓이면 어느 게 아직 살아 있는 요청인지 알 수 없습니다. 회신이 와도 **요청 본문은 고치지 않고** 배너만 바꿉니다 — 당시 무엇을 물었는지가 기록이기 때문입니다. 형식은 `request/frontend/README.md` "회신 상태 표시 규칙" 참조.
...
- **이 폴더의 스펙·계약 문서를 고쳤으면, 같은 작업 안에서 각자의 실행 계획 문서도 확인하세요.** 파생 문서는 자동으로 따라오지 않습니다 — 확인하지 않으면 조용히 낡고, 그 상태로 구현하면 문서끼리 어긋난 코드가 나옵니다.
  - 백엔드: `../backend/docs/` — 어느 문서가 어느 Phase에 대응하는지는 `../backend/docs/README.md`의 **"공용 문서가 바뀌면 이 폴더를 확인한다"** 표에 있습니다
  - AI·프론트도 각자 실행 문서를 두게 되면 같은 규칙을 적용하세요
  - **2026-08-25에 이 규칙이 없어 백엔드 문서 6개가 하루치만큼 낡았습니다.** Phase 5가 Phase 4와 다른 구조를 전제하게 되어, 그대로 구현했으면 사용자가 Stage 3과 Stage 4에서 서로 다른 체크리스트를 보게 될 뻔했습니다
